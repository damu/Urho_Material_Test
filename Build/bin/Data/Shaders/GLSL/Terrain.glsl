...
void PS()
{
    ...
    weights.g*=texture(sDetailMap1, vec3(vDetailTexCoord,10)).r;
    weights.b*=texture(sDetailMap1, vec3(vDetailTexCoord,11)).r;
    ...
    // diffuse mapping
    vec4 diffColor = vec4(
        cMatDiffColor1 * weights.r * texture(sDetailMap1, vec3(vDetailTexCoord,0)) +
        cMatDiffColor2 * weights.g * texture(sDetailMap1, vec3(vDetailTexCoord,1)) +
        cMatDiffColor3 * weights.b * texture(sDetailMap1, vec3(vDetailTexCoord,2))
    );
//    vec4 diffColor=vec4(1,1,1,1);

    // normal mapping
    vec3 normal=normalize(mat3(vTangent.xyz, vec3(vTexCoord.zw, vTangent.w), vNormal)*DecodeNormal(
        weights.r * texture(sDetailMap1, vec3(vDetailTexCoord,3)) +
        weights.g * texture(sDetailMap1, vec3(vDetailTexCoord,4)) +
        weights.b * texture(sDetailMap1, vec3(vDetailTexCoord,5))
    ).rgb);
//vec3 normal=vNormal;

    // specular mapping
    vec4 specColor=vec4(
        weights.r * cMatSpecColor1.rgb * texture(sDetailMap1, vec3(vDetailTexCoord,6)).rgb +
        weights.g * cMatSpecColor2.rgb * texture(sDetailMap1, vec3(vDetailTexCoord,7)).rgb +
        weights.b * cMatSpecColor3.rgb * texture(sDetailMap1, vec3(vDetailTexCoord,8)).rgb,
